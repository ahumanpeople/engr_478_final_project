...
void ADC_Common_Configuration()
{
	...
	ADC123_COMMON->CCR |=  ADC_CCR_CKMODE_0; // set CKMODE[1:0] to ë01í
...
//  One external input channel: PB0 (ADC12_IN15)
//-------------------------------------------------------------------------------------------
void ADC_Pin_Init(void)
{
	...
	//    Configure PB0 (ADC12_IN15) as Analog mode
	GPIOB->MODER |= 0x3UL<<(2U*POTEN);
...
void ADC_Init(void)
{
	...
	ADC1->DIFSEL &= ~ADC_DIFSEL_DIFSEL_15; 	// Select single-ended mode for PB0 (ADC12_IN15)
	...
	// For L4: channel 10..18 use SMPR2; SMP15 is 3 bits.
	// Choose a moderate value like 47.5 cycles (good for pots)
	ADC1->SMPR2 &= ~ADC_SMPR2_SMP15;
	ADC1->SMPR2 |=  ADC_SMPR2_SMP15_2;  // 0b100 -> 47.5 cycles
	...
	//    In this sample, one analog input channel PB0 (ADC12_IN15) is used as Input Channel 15 for ADC1.
	ADC_Pin_Init();
	...
	ADC1->SQR1 |= (ADCWIRE << 6);   // Set channel 15 (connected to PB0) for the 1st conversion
